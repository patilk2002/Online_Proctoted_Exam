...
header("location:Admin_dash.php?q=3");
}
}
...
header("location:Admin_.php?q=1");
}
}
//remove quiz
if(isset($_SESSION['key'])){
...
header("location:Admin_dash.php?q=5");
}
}
...
header("location:Admin_dash.php?q=4&step=2&eid=$id&n=$total");
}
}
...
{
$sn++;
header("location:home.php?q=quiz&step=2&eid=$eid&n=$sn&t=$total")or die('Error152');
}
...
$sun=$sun-$s;
$q=mysqli_query($con,"UPDATE `rank` SET `score`=$sun ,time=NOW() WHERE email= '$email'")or die('Error174');
header("location:home.php?q=quiz&step=2&eid=$eid&n=1&t=$t");
}
