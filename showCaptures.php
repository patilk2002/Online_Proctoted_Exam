...
<body>
	...
	<div class="regitable" align="center">
		...
		<table id="myTable" style="width: 95%">
			...
			<tbody>
				<?php
				$sql = "SELECT history.email, user.name,quiz.title,history.score,history.date,quiz.eid FROM user INNER JOIN history ON history.email=user.email INNER JOIN quiz ON quiz.eid=history.eid ";
				...
				while ($row = mysqli_fetch_assoc($result)) { ?>
					<tr>
						...
						<td scope='row'><button type='submit'  onclick="playVid('<?php echo $row['email'],$row['eid'],$row['date'] ?>','<?php echo $row['name'] ?>')" 
